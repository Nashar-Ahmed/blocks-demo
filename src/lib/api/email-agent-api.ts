...
const API_BASE_URL = 'https://emailagent.duckdns.org';
...
class EmailAgentAPI {
  ...
  // Reply to email
  async replyToEmail(emailId: string, body: string, tone = 'professional') {
  ...
  async generateResponse(emailId: string, tone = 'professional', messageId?: string) {
  ...
  // Chat endpoints
  async sendChatMessage(message: string, includeContext = true) {
    return this.request<{
      success: boolean;
      response: string;
      tokens_used?: number;
  ...
  async semanticSearch(query: string, nResults = 10) {
    return this.request<{
      success: boolean;
      query: string;
      results: Array<{
  ...
  async loadToVector(dateFrom?: string, dateTo?: string, limit = 1000) {
    const params = new URLSearchParams({ limit: limit.toString() });
    if (dateFrom) params.append('date_from', dateFrom);
    if (dateTo) params.append('date_to', dateTo);
  ...
  async analyzeExistingEmails(limit = 100) {
    return this.request<{
      success: boolean;
      processed: number;
      message: string;
